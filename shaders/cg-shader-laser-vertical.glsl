...
const float LINE_THICKNESS = 2.0; // Adjust this value to change line thickness
const float GLOW_INTENSITY = 0.8; // Adjust glow strength
const float GLOW_SIZE = 20.0; // Adjust glow spread
...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
    ...
    // Distance between cursors determine the total length of the line
    float lineLength = distance(centerCC, centerCP);

    vec4 newColor = vec4(fragColor);

    // Yellow glow color
    vec3 glowColor = vec3(1.0, 0.95, 0.3); // Bright yellow
    
    // Calculate glow for trail
    float glowSize = norm(vec2(GLOW_SIZE, GLOW_SIZE), 0.).x;
    float trailGlow = exp(-abs(sdfTrail) / glowSize) * GLOW_INTENSITY;
    
    // Calculate glow for cursor
    float cursorGlow = exp(-abs(sdfCurrentCursor) / glowSize) * GLOW_INTENSITY;
    
    // Combine glows
    float totalGlow = max(trailGlow, cursorGlow);
    
    // Apply glow
    newColor.rgb += glowColor * totalGlow;
